...
class Struct:
    ...
    def __cmp__(self, other):
        if isinstance(other, Struct):
            return __cmp__(self.__dict__, other.__dict__)
        else:
            return __cmp__(self.__dict__, other)
...
class heuristica(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key=lambda node: node.path_cost + search.GPSProblem.h(self.problem, node))
